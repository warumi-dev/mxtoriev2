        ...
        await CmdExist(database, message, this.help.name).then(async () => {
            await database.query("SELECT * FROM commands WHERE serverid = ? AND name = ?", [message.guild.id, this.help.name], async (error, result) => {
                ...
                try {
                    ...
                    var values = [args[0], message.guild.id];
                    await database.query('UPDATE settings SET prefix = ? WHERE serverid = ? ', values, async function (error, result, fields) {
                        if (error || result < 1) return message.reply(lang.error + error)
                        return message.channel.send(lang.newprefix + ' ' + args[0])
                    })
...
module.exports.help = {
    name: "setprefix",
    aliases: [`prefix`, `cp`],
    desc: ["Change le prefix du bot", "Change the bot prefix"],
    access_member: [""],
