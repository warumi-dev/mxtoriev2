        ...
        await CmdExist(database, message, this.help.name).then(async () => {
            await database.query("SELECT * FROM commands WHERE serverid = ? AND name = ?", [message.guild.id, this.help.name], async (error, result) => {
                ...
                try {
                    ...
                    lang = language[`${lang2}`]
                    let author = await db.fetch(`name_${message.guild.id}`)
                    ...
                    if (author !== null && timeout - (Date.now() - author) > 0) {
                        ...
                        if (args.length) {
                            ...
                            client.user.setUsername(str_content)
                                .then(u => {
                                    message.channel.send(`\`${getNow().time}\` :white_check_mark: ${message.author}, ${lang.by == 'by' ? 'Name changed for : ' + str_content : 'Nom changé pour : ' + str_content}`)
                                    db.set(`name_${message.guild.id}`, Date.now())
                                })
...
module.exports.help = {
    name: "name",
    aliases: [`username`, `setname`],
    desc: ["Change le pseudo du bot", "Change the username of the bot"],
    access_member: [""],
