                ...
                try {
                    ...
                    if (author !== null && timeout - (Date.now() - author) > 0) {
                        ...
                        let timeEmbed = new Discord.MessageEmbed()
                            .setColor("#FFFFFF")
                            .setDescription(`❌ Tu as déjà récupèrer ta paye du jour.\n\nRé-essayer dans : ${pretty(ms(time))} `);
                        ...
                        let amounta = Math.floor(Math.random() * 400) + 1;
                        let multiplier = await db.fetch(`multiplierdaily_${message.guild.id}`);
                        if (!multiplier) multiplier = 1;
                        let amount = amounta * multiplier;

                        let embed1 = new Discord.MessageEmbed()
                            .setColor("#FFFFFF")
                            .setDescription(`✅ - Tu as récupèrer ta paye du jour pour gagné ${amount} coins !`);
...
module.exports.help = {
    name: "daily",
    aliases: [`day`],
    desc: ["Reçois ta paye du jour", "Get you money of the day"],
    access_member: [""],
