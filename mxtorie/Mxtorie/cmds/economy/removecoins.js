        ...
        await CmdExist(database, message, this.help.name).then(async () => {
            await database.query("SELECT * FROM commands WHERE serverid = ? AND name = ?", [message.guild.id, this.help.name], async (error, result) => {
                ...
                try {
                    ...
                    if(!args[1] || isNaN(args[1])) return embed.helpargs(client, message, this.help.name + " - " + lang.invalidargs, this.help.usage, prefix)
                    database3.query("SELECT * FROM coins WHERE serverid = ? AND userid = ?", [message.guild.id, mention], async(error, result) => {
                        if(error) return message.channel.send(lang.error+error)
                        let currentcoins = parseInt(result[0].usercoins)
                        await database3.query("UPDATE coins SET usercoins = ? WHERE serverid = ? AND userid = ?", [currentcoins-parseInt(args[1]), message.guild.id, mention], async(error, result) => {
                            if(error) return message.channel.send(lang.error+error)
                            return message.channel.send(`**${args[1]}coins** ont bien été retiré à <@${mention}> (${mention}) qui a maintenant **${currentcoins-parseInt(args[1])}coins**.`)
                        })
                    })
...
module.exports.help = {
    name: "removecoins",
    aliases: [`removecoin`],
    desc: ["Retirer des coins à un membre", "Remove coins to a member"],
    access_member: [""],
    access_bot: ["SEND_MESSAGES", "EMBED_LINKS"],
    usage: ["removecoins <mention/id> <valeur>"],
