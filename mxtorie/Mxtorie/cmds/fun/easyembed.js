        ...
        await CmdExist(database, message, this.help.name).then(async () => {
            await database.query("SELECT * FROM commands WHERE serverid = ? AND name = ?", [message.guild.id, this.help.name], async (error, result) => {
                ...
                try {
                    ...
                    let mycolor = args[0].toUpperCase()
                    let text = args.splice(1).join(' ')
                    if (!text) return embed.helpargs(client, message, this.help.name + " - " + lang.invalidargs, this.help.usage, prefix)
                    let embed2 = new Discord.MessageEmbed()
                    embed2.setColor(mycolor)
...
module.exports.help = {
    name: "easyembed",
    aliases: [`simpleembed`, `embedsimple`, `embedeasy`],
    desc: ["Crée un embed très simpliste", "Create a easy embed"],
    access_member: [""],
    access_bot: ["SEND_MESSAGES", "EMBED_LINKS"],
    usage: ["easyembed <couleur(en anglais)> <votre texte>"],
    type: ["Fun", "Fun"],
    perm: "owner"
}
