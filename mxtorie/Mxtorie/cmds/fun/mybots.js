        ...
        await CmdExist(database, message, this.help.name).then(async () => {
            await database.query("SELECT * FROM commands WHERE serverid = ? AND name = ?", [message.guild.id, this.help.name], async (error, result) => {
                ...
                try {
                    ...
                    const cRoWbOt = await new Discord.MessageAttachment("./crow.jpg")
                        let embed2 = await new Discord.MessageEmbed()
                        embed2.setTitle("Mybots")
                        embed2.setDescription("Je suis pas un crow bot moi.")
                        embed2.attachFiles(cRoWbOt)
                        embed2.setImage("attachment://crow.jpg")
...
module.exports.help = {
    name: "mybots",
    aliases: [`mybot`],
    desc: ["Affiche vos abonnements", "Show you subs"],
    access_member: [""],
