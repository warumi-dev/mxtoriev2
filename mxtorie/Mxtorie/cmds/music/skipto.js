        ...
        await CmdExist(database, message, this.help.name).then(async () => {
            await database.query("SELECT * FROM commands WHERE serverid = ? AND name = ?", [message.guild.id, this.help.name], async (error, result) => {
                ...
                try {
                    ...
                    if (!queue) return sendError("Il n'y a aucune file d'attente.", message.channel).catch(console.error);
                    ...
                    try {
                        ...
                        return sendError(`:notes: La musique a été arrêté et la file d'attente vidé : ${error}`, message.channel);
                    }

                    queue.textChannel.send({
                        embed: {
                            color: "GREEN",
                            description: `${message.author} ⏭ J'ai sauté \`${args[0] - 1}\` musique(s).`
                        }
...
module.exports.help = {
    name: "skipto",
    aliases: [`st`],
    desc: ["Passe à la musique séléctionner", "Skip to the selected music"],
    access_member: [""],
