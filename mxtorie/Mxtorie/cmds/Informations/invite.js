        ...
        await CmdExist(database, message, this.help.name).then(async () => {
            await database.query("SELECT * FROM commands WHERE serverid = ? AND name = ?", [message.guild.id, this.help.name], async (error, result) => {
                ...
                try {
                    ...
                    const userInvites = invites.array().filter(e => e.inviter.id == mention);
                    ...
                    embed2.setDescription(`<@${mention}> a actuellement ${userInvites.length} lien(s) actif. Au total il a invité ${inviteCount} membre(s).`);

                    //embed.setDescription(lang.by=='by' ? `<@${mention}> have a total of ${invite.uses} invite(s).` : `<@${mention}> a un total de ${invite.uses} invite(s).`)
...
module.exports.help = {
    name: "invite",
    aliases: [`invites`, `inv`, `invitation`, `invitations`],
    desc: ["Montre combien d'invitation a un membre", "description en"],
    access_member: [""],
    access_bot: ["SEND_MESSAGES", "EMBED_LINKS"],
    usage: ["invite [mention/id]"],
