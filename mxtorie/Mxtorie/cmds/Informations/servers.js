        ...
        await CmdExist(database, message, this.help.name).then(async () => {
            await database.query("SELECT * FROM commands WHERE serverid = ? AND name = ?", [message.guild.id, this.help.name], async (error, result) => {
                ...
                try {
                    ...
                    embed2.setDescription(`\`\`\`json\n${client.guilds.cache.filter(g=>g.id!='855557733389959188').map((g,n) => n==1?`${g.id} : ${g.name}` : `\n${g.id} : ${g.name}`)}\`\`\``)
...
module.exports.help = {
    name: "servers",
    aliases: [`server`, `serverlist`, `serverslist`],
    desc: ["Affiche les serveurs sur lequels ce trouve le bot", "description en"],
    access_member: [""],
