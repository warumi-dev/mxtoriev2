        ...
        await CmdExist(database, message, this.help.name).then(async () => {
            await database.query("SELECT * FROM commands WHERE serverid = ? AND name = ?", [message.guild.id, this.help.name], async (error, result) => {
                ...
                try {
                    ...
                    var str_filtrer = members.filter(member => member.user.bot)
                    ...
                    for (let i = 0; i < str_map.length; i += 1995) {
                        ...
                        var embed2 = new Discord.MessageEmbed()
                            .setTitle(lang.by == 'by' ? `Bots list (**${str_filtrer.size}**) on the server **${message.guild.name}**` : `Liste des bots (**${str_filtrer.size}**) sur le serveur **${message.guild.name}**`)
                            .setDescription(`\`\`\`json\n${str_content}\`\`\``)
                            .setColor(color.blue)
...
module.exports.help = {
    name: "botlist",
    aliases: [`bots`, `bot`, `botslist`, `listbot`, `listbots`],
    desc: ["Affiche tout les bots présent sur le serveur", "Show all bots on the server"],
    access_member: [""],
