            ...
            await database.query("SELECT * FROM commands WHERE serverid = ? AND name = ?", [message.guild.id, this.help.name], async (error, result) => {
                ...
                try {
                    ...
                    await database.query("SELECT * FROM roles WHERE serverid = ?", message.guild.id, async function (error, result, fields) {
                        ...
                        embed.simple(client, message, fr ? 'Salon débloqué' : 'Channel unlocked', message.guild.iconURL({ dynamic: true }), fr ? `🔓 - Salon ${channel} débloqué.` : `🔓 - Channel ${channel} unlocked.`, color.greenyellow, message.channel)
                        logs.sanctions(client, message, database, fr ? "Salon débloqué" : "Channel unlocked", lang.unlocked, message.guild.name, false, color.salmon, lang, message.author, 'channel', false, channel.id)
                    })
...
module.exports.help = {
    name: "unlock",
    aliases: [``],
    desc: ["Débloque l'accès à un salon", "Unlock a channel"],
    access_member: [""],
