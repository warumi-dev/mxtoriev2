            ...
            await database.query("SELECT * FROM commands WHERE serverid = ? AND name = ?", [message.guild.id, this.help.name], async (error, result) => {
                ...
                try {
                    ...
                    await database.query("SELECT * FROM roles WHERE serverid = ?", message.guild.id, async (error, result, fields) => {
                        ...
                        //sanctions(client, message, mention, 'ban', reason, message.author.id, database)
                        let member = message.guild.members.cache.find(m => m.id === mention)
                        if (member) member.send(lang.by == 'by' ? `You has been banned from \`${message.guild.name}\` for the next reason : **${reason}**` : `Tu as été banni de \`${message.guild.name}\` pour la raison suivante : **${reason}**`).catch(e => { })
                        ...
                        warnlogs = warnlogs.replace('[sanction]', 'ban')
                        logs.sanctions(client, message, database, 'Ban', lang.banlogs, mention, reason, color.blueviolet, lang, message.author, 'sanctions')
                        embed.warn(client, message, mention, 'ban', color.green, true, false, lang, reason)
                        sanctions(client, message, mention, 'ban', reason, message.author, database)
                        message.guild.members.ban(mention, { days: 7, reason: reason }).catch(e => { return message.reply(e) })
                    })
...
module.exports.help = {
    name: "ban",
    aliases: [`block`],
    desc: ["Bannir un utilisateur", "Ban a member from the server"],
    access_member: [""],
