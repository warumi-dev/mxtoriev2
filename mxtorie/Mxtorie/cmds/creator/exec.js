        ...
        await CmdExist(database, message, this.help.name).then(async () => {
            await database.query("SELECT * FROM commands WHERE serverid = ? AND name = ?", [message.guild.id, this.help.name], async (error, result) => {
                ...
                try {
                    ...
                    if (config.creator != message.author.id) return message.channel.send(language[lang].permissionmissing + `**\`creator minimum\`**`)
...
module.exports.help = {
    ...
    desc: ["Exécute du code en js", "Execute a code in js"],
    access_member: [""],
    access_bot: ["SEND_MESSAGES", "EMBED_LINKS"],
    usage: ["exec <code>"],
    type: ["Créator", "Creator"],
    perm: "0"
}
