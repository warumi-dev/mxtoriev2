            ...
            await database.query("SELECT * FROM commands WHERE serverid = ? AND name = ?", [message.guild.id, this.help.name], async (error, result) => {
                ...
                try {
                    ...
                    await database.query("SELECT * FROM settings WHERE serverid = ?", message.guild.id, async function(error, result, fields) {
                        if (error || result < 1) return message.reply(lang.undefinederror)
                        var state = result[0].support
                        var msg = result[0].supportmsg
                        await database.query("SELECT * FROM roles WHERE serverid = ?", message.guild.id, async function(error, result, fields) {
                            if (error || result < 1) return message.reply(lang.undefinederror)
                            ...
                            if (!message.guild.roles.cache.has(role)) {
                                ...
                                role = message.guild.roles.cache.find(r => r.id === role).name
                            }
                            let embed2 = new Discord.MessageEmbed()
                            embed2.setTitle("Support")
                            embed2.addField("État :", state == 'on' ? `\`activer\`` : `\`désactiver\``, false)
                            embed2.addField('Rôle :', `\`${role}\``, false)
                            embed2.addField("Pour l'obtenir :", `\`${msg}\``, false)
                            embed2.setColor(color.limegreen)
                            embed2.setFooter(config.web)
...
module.exports.help = {
    name: "support",
    aliases: [`soutiens`, `sprt`],
    desc: ["Montre le statut du rôle soutiens", "Show you the support role"],
    access_member: [""],
